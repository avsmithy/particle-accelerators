...
Each electron beam loses about 4 GeV in energy, which can be is compensated for by increasing the voltages of the two linacs from 10 GV to 10.63 GV. The beams in the 70 GeV arc lose the most energy due to synchrotron radiation: 1.39 GeV, or a 2\% loss \cite{Bogacz:SAPPHiRE}.
...
The initial normalised beam emittances quoted in the sapphire paper of 5 μm and 0.5 μm in the \textit{x} and \textit{y} directions respectively, correspond to best unpolarised RF gun emittances but photon colliders need polarised electrons. Currently no low emittance polarised RF guns exist (though progress is being made) \cite{Telnov:Photons:MIT}.
